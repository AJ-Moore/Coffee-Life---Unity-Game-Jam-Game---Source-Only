...
public class Eggtastic : MonoBehaviour {
	...
	void Start () {
		...
		if (this.director != null) {
			//!< Check if this egg had already been collected... 
			if (this.director.levelEggCollected [Application.loadedLevel-1] == true) {
				this.director.eggsCollected = 1; 
	...
	void OnTriggerEnter2D(Collider2D other) {
		if (other.gameObject.tag == "Player") {
			...
			if ( this.director != null){
				this.director.eggsCollected++; 
				this.director.totalEggsCollected++;
				this.director.levelEggCollected[Application.loadedLevel-1] = true;
			}
	...
	//!< Prevents the egg from being rendered on responding to collision. 
	void disableEgg(){
